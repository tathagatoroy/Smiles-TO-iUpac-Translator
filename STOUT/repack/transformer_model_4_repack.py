...
def scaled_dot_product_attention(queries, keys, values, mask):
    ...
    # scale matmul_qk
    dim_keys = tf.cast(tf.shape(keys)[-1], tf.float32)
    scaled_attention_logits = matmul_qk / tf.math.sqrt(dim_keys)
...
def point_wise_feed_forward_network(d_model, dff):
    """ simulates networks that performs pointwise feedforward
    Args :
        dff : dimension
...
class EncoderLayer(tf.keras.layers.Layer):
    """ class to implement the Encoder network"""
    def __init__(self, d_model, num_heads, dff, rate=0.1):
        """ models the Encoder network """
    ...
    def call(self, input_x, training, mask):
        ...
        out1 = self.layernorm1(input_x + attn_output)  # (batch_size, input_seq_len, d_model)
...
class Decoder(tf.keras.layers.Layer):
    """ implements the decoder network"""
    def __init__(
        self,
        num_layers,
        d_model,
